...
library(lubridate)
NTL.phys.data <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE)     #SaF, otherwise this would've imported strings as characteR? It is instead changed them to  factors
...
newdf <- filter(NTL.phys.data, year4 == 1999) 
newdf
...
newdf2 <- newdf %>% 
  filter(depth == 0) %>% 
  summarise(mean_surface_temp = mean(temperature_C), na.rm = TRUE)
...
d.long <- NTL.phys.data %>% 
  pivot_longer (cols=9:10, names_to =
                  "irradiance", values_to = "irradiance_value")


#Exercise 6: spread temperatureC into more than one column based on the depth
d.wide <- pivot_wider(NTL.phys.data, names_from = temperature_C)
