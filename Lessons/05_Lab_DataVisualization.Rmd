...
temp_scatter_plot <- ggplot(PeterPaul.chem.nutrients, aes(x = daynum, y = temperature_C))+
  geom_point(aes(color = year4))+
  xlab("Day of the Year")+ #changes x axis label 
  ylab("Temperature C")+
  
  facet_wrap(vars(lakename) ,nrow=2)
print(temp_scatter_plot)
#2. 
...
temp_by_date <- ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C))+
  geom_point(aes(color = depth), size = 0.5)+ 
  scale_color_viridis(option = "magma", direction = -1)+
  scale_x_date(limits=as.Date(c("1984-05-27", "2014-08-29")), breaks = "5 years", date_labels = "%y")
...
EPAair_plot <- ggplot(EPAair, aes(x= PM2.5, y=Ozone, color = SITE_LATITUDE))+
...
airplot1 <- ggplot(EPAair, aes(x= as.factor(Year), y = PM2.5))+
  geom_violin()
airplot1
