...
inclass_reg <- lm(data = EPAair, PM2.5 ~  Ozone)
summary(inclass_reg)

          #pvalue of fstat: <0.05  ; R2 = .1316
...
scatter_inclass <- ggplot(EPAair, aes(x=Ozone, y=PM2.5))+
  geom_point()+
  geom_smooth(method=lm, color = "blue")
print(scatter_inclass)
...
corrplot_inclass <- 
  ...
  na.omit()
corrplot_inclass2 <- cor(corrplot_inclass)

corrplot(corrplot_inclass2, method = "ellipse")
corrplot.mixed(corrplot_inclass2, upper = "ellipse")
...
AIC_inclass <- lm(data = corrplot_inclass, PM2.5 ~ SITE_LATITUDE + SITE_LONGITUDE + Month + Year + Ozone)   #should i use this or the next one below "AIC_lakevars2"  ?? 
step(AIC_inclass)
...
AIC_inclass2 <- lm(data = corrplot_inclass, PM2.5 ~ SITE_LATITUDE + SITE_LONGITUDE + Month + Ozone)
summary(AIC_inclass2)


#R2 = .1926; fstat p value < 0.05 
# the r2 in this model is greater than the other
```
