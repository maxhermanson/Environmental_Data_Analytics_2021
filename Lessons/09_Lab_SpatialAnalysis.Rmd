...
HUC8_UN_sf <- st_read('./Data/Spatial/NCHUC8.shp') %>% 
  filter(SUBBASIN == 'Upper Neuse')

mapview(huc8_sf, col.regions = 'orange')+mapview(HUC8_UN_sf)
...
wind_energy_huc8 <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12'= 'factor')) %>% 
  mutate('HUC_8' = substr(HUC_12, 1, 8)) %>% #start at first character and grab through the 8th
  group_by(HUC_8) %>% 
  summarise(power=mean(kWhkm2day))
#Join to HUC_8 features
new_join <-  huc8_sf %>% 
  left_join(wind_energy_huc8, by = NULL) %>% 
  drop_na()



#View the outputs
mapview(new_join, zcol='kWhkm2day')
...
mapview(the_huc_clip)+mapview()+mapview() # incomplete code SLS
