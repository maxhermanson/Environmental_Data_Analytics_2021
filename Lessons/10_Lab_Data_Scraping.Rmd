...
alter 'theURL' ; specifcally change the no. and the period = 24 instead of period = 7 
## APIs
...
```{r}
#Apply the API key
census_api_key("414e305370e6f639b15399797939b19886ad4e66", install = TRUE)
...
df_website <- data.frame(state = the_states, 
                         price_c_kwh = the_price, 
...
the_df <- data.frame(
  state = the_states, 
  price_c_kwh = as.numeric(gsub(",","", the_price)),
  capacity = as.numeric(gsub)) %>% ### need to complete this
...
df_all <- map(rep(2017:2019), scrape.it) %>% bind_rows()
